...
func threadJob(context threadContext, c chan threadContext) {
	...
	rhsBuf := make([]uint16, _ARITH_MAX_RHS_LEN)
	rhsSymbolsBuf := make([]*symbol, _ARITH_MAX_RHS_LEN)
...
/*
Parse parses a file in parallel using an operator precedence grammar.
It takes as input a filename and the number of threads, and returns a boolean
representing the success or failure of the parsing and the value at the root of the syntactic tree (if successful).
*/
func Parse(filename string, numThreads int) (bool, int64) {
	...
	stackPool := newStackPool(10000)

	stackPtrPool := newStackPtrPool(1000)
	...
	input, err := lex(filename, stackPool)

	fmt.Printf("Time to lex %s: %s\n", filename, time.Since(start))
	...
	if numThreads > 1 {
		...
		//Set the result as the value of the nonterminal
		result = *sym.Value.(*int64)
	} else {
		//Pop tokens from the stack until a nonterminal is found
